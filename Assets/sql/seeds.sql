--File added as a reference for templates within JS functions

SELECT role.id, role.title, role.salary FROM role ORER BY role.id;
...
SELECT department.name AS department, role.title, employee.id, employee.first_name, employee.last_name
    FROM employee
    LEFT JOIN role ON (role.id = employee.roll_id)
